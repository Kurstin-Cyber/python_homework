...
try:
    ...
    for prod_id in product_ids:
        cursor.execute(
            "INSERT INTO line_items (order_id, product_id, quantity) VALUES (?, ?, ?);",(new_order_id, prod_id, 10)
        )
    ...
    for item in verification_results:
        li_id, qty, prod_name, price = item
        print(f" - Line Item ID: {li_id} | Product: {prod_name} | Qty: {qty} | Unit Price: ${price:.2f}")
